        ...
        <div className='flex justify-center w-full relative max-w-screen-2xl mx-auto bg-black/50 z-[60] text-white border-b border-stone-700 border-opacity-35 backdrop-blur-sm'>
            <div className='flex items-center justify-between w-full px-4 fixed'>
                <div className='flex items-center gap-5 pl-1'>
                    ...
                    <Link href={'/docs'}>
                        <div className={` ${path == '/docs' ? "brightness-105" : "brightness-50"}  hover:brightness-105 transition-all text-sm md:text-base   duration-200 hidden md:block cursor-pointer font-sans font-normal tracking-tight`}>Docs</div>
                    </Link>
                    <Link href={'/docs/components/Navbar'}>
                        <div className={` ${path.includes('/docs/components') ? "brightness-105" : "brightness-50"} hover:brightness-105 text-sm md:text-base   transition-all duration-200 hidden md:block cursor-pointer font-sans font-normal tracking-tight`}>Components</div>
                    </Link>
                    <Link href={'/templates'}>
                        <div className={` ${path == '/templates' ? "brightness-105" : "brightness-50"} hover:brightness-105 transition-all text-sm md:text-base   duration-200 hidden md:block cursor-pointer font-sans font-normal tracking-tight`}>Templates</div>
                    </Link>
                    <Link href={'/contact'}>
                        <div className={` ${path == '/contact' ? "brightness-105" : "brightness-50"} hover:brightness-105 transition-all text-sm md:text-base   duration-200 hidden md:block cursor-pointer font-sans font-normal tracking-tight`}>Contact</div>
                    </Link>
                </div>
                <div className='flex items-center gap-4 px-3 py-4'>
                    <Modal>
                        <ModalTrigger>
                            <div className='hidden lg:flex md:gap-4 brightness-75 hover:brightness-105 cursor-pointer font-sans text-sm border border-gray-500 px-4 py-1 rounded-lg
